...
<<<<<<< HEAD static const enum AVPixelFormat mmal_pixfmts[] = {
    AV_PIX_FMT_MMAL, AV_PIX_FMT_YUV420P, AV_PIX_FMT_NONE};

static void ffmmal_poolref_unref(FFPoolRef *ref) {
  if (ref && atomic_fetch_add_explicit(&ref->refcount, -1,
...
>>>>>>> 931cd5d32e (rebase)
  ...
  static int ffmmal_set_ref(AVFrame * frame, FFPoolRef * pool,
    ...
    frame->buf[0] =
        av_buffer_create((void *)ref, sizeof(*ref), ffmmal_release_frame, NULL,
  ...
  static int ffmal_update_format(AVCodecContext * avctx) {
    ...
    if ((status = mmal_port_parameter_set_uint32(decoder->output[0],
                                                 MMAL_PARAMETER_EXTRA_BUFFERS,
                                                 ctx->extra_buffers)))
      goto fail;

    if ((status = mmal_port_parameter_set_boolean(
             decoder->output[0], MMAL_PARAMETER_VIDEO_INTERPOLATE_TIMESTAMPS,
             0)))
      goto fail;
...
>>>>>>> 931cd5d32e (rebase)
    ...
    if (mmal_port_parameter_set_uint32(decoder->input[0],
  ...
  static int ffmmal_add_packet(AVCodecContext * avctx, AVPacket * avpkt,
    ...
    do {
      ...
      buffer->pts =
          avpkt->pts == AV_NOPTS_VALUE ? MMAL_TIME_UNKNOWN : avpkt->pts;
      buffer->dts =
          avpkt->dts == AV_NOPTS_VALUE ? MMAL_TIME_UNKNOWN : avpkt->dts;
  ...
  static int ffmal_copy_frame(AVCodecContext * avctx, AVFrame * frame,
    ...
    frame->pts =
        buffer->pts == MMAL_TIME_UNKNOWN ? AV_NOPTS_VALUE : buffer->pts;
  ...
  static int ffmmal_read_frame(AVCodecContext * avctx, AVFrame * frame,
    ...
    while (1) {
      // To ensure decoding in lockstep with a constant delay between fed
      // packets and output frames, we always wait until an output buffer is
      // available. Except during start we don't know after how many input
      // packets the decoder is going to return the first buffer, and we can't
      // distinguish decoder being busy from decoder waiting for input. So just
      // poll at the start and keep feeding new data to the buffer. We are
      // pretty sure the decoder will produce output if we sent more input
      // frames than what a H.264 decoder could logically delay. This avoids too
      // excessive buffering. We also wait if we sent eos, but didn't receive it
      // yet (think of decoding stream with a very low number of frames).
  ...
  static const AVCodecHWConfigInternal *const mmal_hw_configs[] = {
      HW_CONFIG_INTERNAL(MMAL), NULL};

  static const AVOption options[] = {
      {"extra_buffers",
       "extra buffers",
       offsetof(MMALDecodeContext, extra_buffers),
       AV_OPT_TYPE_INT,
       {.i64 = 10},
       0,
       256,
      ...
      {"extra_decoder_buffers",
       "extra MMAL internal buffered frames",
       offsetof(MMALDecodeContext, extra_decoder_buffers),
       AV_OPT_TYPE_INT,
       {.i64 = 10},
       0,
       256,
...
#define FFMMAL_DEC(NAME, ID)                                                   \
  const FFCodec ff_##NAME##_mmal_decoder = {                                   \
      ...
      .caps_internal = FF_CODEC_CAP_NOT_INIT_THREADSAFE,                       \
      .p.pix_fmts =                                                            \
          (const enum AVPixelFormat[]){AV_PIX_FMT_MMAL, AV_PIX_FMT_YUV420P,    \
                                       AV_PIX_FMT_NONE},                       \
      .hw_configs = mmal_hw_configs,                                           \
      .p.wrapper_name = "mmal",                                                \
  };
